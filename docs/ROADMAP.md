...
- Single-file compiled executable — Linux only, confirmed working
  standalone with no Python install needed
...
- **Windows server support** — nothing built (LAN detection, vendored
  Ollama, hotspot instructions all Linux/Mac only so far).
- **Compiled single-file executable for Mac/Windows** — only Linux
  exists.
