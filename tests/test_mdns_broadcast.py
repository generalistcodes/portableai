...
def test_run_py_advertises_on_startup_and_unregisters_on_ollama_shutdown_path():
    ...
    assert "advertiser.start(args.port, version=server.APP_VERSION)" in text
...
def test_live_browse_finds_two_distinct_named_servers():
    """Real multicast: Ubuntu-named and Mac-named entries both visible."""
    ubuntu = MdnsAdvertiser()
    mac = MdnsAdvertiser()
    try:
        assert ubuntu.start(15901, hostname="gg-G5-KC", version=APP_VERSION)
        assert mac.start(15902, hostname="Kims-MacBook-Air", version=APP_VERSION)
        found = _wait_for(
            lambda snap: {v["txt"].get("name") for v in snap.values()}
            >= {"gg-G5-KC", "Kims-MacBook-Air"}
        )
        names = {entry["txt"]["name"]: entry for entry in found.values()}
        assert "gg-G5-KC" in names, found
        assert "Kims-MacBook-Air" in names, found
        assert names["gg-G5-KC"]["txt"]["version"] == APP_VERSION
        assert names["Kims-MacBook-Air"]["txt"]["version"] == APP_VERSION
        assert names["gg-G5-KC"]["port"] == 15901
        assert names["Kims-MacBook-Air"]["port"] == 15902
    finally:
        ubuntu.stop()
        mac.stop()
