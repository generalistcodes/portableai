...
async function refreshStatus() {
  ...
  try {
    ...
    const modeEl = el("ollamaModeLabel");
...
async function loadCatalog() {
  try {
    ...
    state.catalog.sort((a, b) => (b.recommended ? 1 : 0) - (a.recommended ? 1 : 0));
  ...
  try {
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        if (ev.message && buttonEl) buttonEl.textContent = ev.message;
...
const PERSONA_DOWNLOAD_ICON =
  '<svg class="persona-download-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v12"/><path d="M7 11l5 5 5-5"/><path d="M5 21h14"/></svg>';

function sizeLabelForModel(name) {
  const hit = (state.catalog || []).find((e) => e.name === name);
  if (!hit || !hit.approx_size) return "";
  const raw = String(hit.approx_size).replace(/^~/, "");
  return `~${raw}`;
}
...
function revealAppShell() {
  document.documentElement.classList.remove("awaiting-first-run");
  setOverlayVisible(el("setupOverlay"), false);
  setOverlayVisible(el("onboardOverlay"), false);
}

function applySetupStatus(status) {
  const overlay = el("setupOverlay");
  const busy = Boolean(status && status.busy);
  setOverlayVisible(overlay, busy);
  if (!busy) return;
  const phase = status.phase || "";
  const labels = {
    downloading_ollama: "Downloading Ollama",
...
async function showOnboardingUntilInstalled() {
  let catalog;
  try {
    catalog = await api("/api/models/catalog");
  } catch {
    return;
  }
  state.catalog = Array.isArray(catalog) ? catalog : [];
  const recommended = state.catalog.filter((e) => e.recommended);
  const choices = recommended.length ? recommended : state.catalog.slice(0, 3);
  if (!choices.length) return;
  const host = el("onboardCatalog");
  if (!host) return;
  host.innerHTML = "";
  choices.forEach((entry) => {
    const row = document.createElement("div");
    row.className = "onboard-option";
    const info = document.createElement("span");
    info.innerHTML = `<span class="model-name">${escapeHtml(entry.name)}</span><br><span class="model-detail">${escapeHtml(entry.family || "")} · ~${escapeHtml(entry.approx_size || "")}<br>${escapeHtml(entry.description || entry.recommended_reason || "")}</span>`;
    const btn = document.createElement("button");
    btn.className = "download-btn";
    btn.type = "button";
    btn.textContent = "Download";
    btn.addEventListener("click", async () => {
      const ok = await downloadModel(entry.name, btn);
      if (ok) setOverlayVisible(el("onboardOverlay"), false);
    });
    row.appendChild(info);
    row.appendChild(btn);
    host.appendChild(row);
  });
  setOverlayVisible(el("onboardOverlay"), true);
  while ((state.models || []).length === 0) {
    await sleep(800);
    await loadModels();
  }
  setOverlayVisible(el("onboardOverlay"), false);
...
async function gateFirstRun() {
  await waitForSetupReady();
  await loadModels();
  if ((state.models || []).length === 0) {
    await showOnboardingUntilInstalled();
  }
  revealAppShell();
}
...
async function loadPersonas() {
  ...
  state.personas.forEach((p) => {
    const btn = document.createElement("button");
    const ready = p.model_installed !== false;
    btn.className = "persona-item" + (p.id === state.activePersona ? " active" : "") + (ready ? "" : " is-unavailable");
    btn.dataset.personaId = p.id;
    btn.type = "button";
    const downloadMark = ready ? "" : PERSONA_DOWNLOAD_ICON;
    btn.innerHTML = `${personaIconSvg(p)}<span class="persona-name">${escapeHtml(personaLabel(p))}</span>${downloadMark}`;
    btn.onclick = () => selectPersona(p.id);
    list.appendChild(btn);
  });
  if (!state.activePersona) {
    const ready = state.personas.filter((p) => p.model_installed !== false);
    const landing = pickDefaultPersona(ready.length ? ready : state.personas);
    if (landing && landing.model_installed !== false) selectPersona(landing.id);
  }
  populateModelSelect();
  renderCardChips();
}
...
async function selectPersona(id) {
  ...
  if (persona && persona.model_installed === false) {
    const size = sizeLabelForModel(persona.base_model);
    const sizeBit = size ? ` (${size})` : "";
    const ok = window.confirm(`This needs ${persona.base_model}${sizeBit} — download it now?`);
    if (!ok) return;
    const succeeded = await downloadModel(persona.base_model, null);
    const latest = (state.personas || []).find((p) => p.id === id);
    if (!succeeded || !latest || latest.model_installed === false) return;
  }
...
let pairingSuccessTimer = null;
let connectedBannerTimer = null;
let lastPairedTokens = null;

const PAIRING_POLL_MS = 5000;
const PAIRING_PANEL_POLL_MS = 1800;
...
function isSettingsPairingPaneOpen() {
  const pane = el("pane-pairing");
  return isModalOpen("settingsModal") && Boolean(pane && !pane.classList.contains("hidden"));
}

function isPairingSurfaceOpen() {
  return isSettingsPairingPaneOpen() || isModalOpen("connectDeviceModal");
}

function activePairingWrap() {
  if (isSettingsPairingPaneOpen()) return el("pairingQrWrap");
  if (isModalOpen("connectDeviceModal")) return el("connectQrWrap");
  return null;
}
...
function startPairingPoll() {
  ...
  const fast = isPairingSurfaceOpen();
  const delay = fast ? PAIRING_PANEL_POLL_MS : PAIRING_POLL_MS;
  pairingPollInterval = setInterval(() => {
    if (isPairingSurfaceOpen()) loadPairedDevices();
    else loadPairingInfo();
  }, delay);
}
...
function syncPairingWatch() {
  const waiting = isPairingSurfaceOpen();
  setPairingListening(waiting);
  if (!waiting) hidePairingSuccess(true);
  if (waiting) {
    startPairingPoll();
    return;
  }
...
function setPairingListening(on) {
  ["pairingQrWrap", "connectQrWrap"].forEach((id) => {
    const wrap = el(id);
    if (!wrap) return;
    const active = Boolean(on) && wrap === activePairingWrap() && !wrap.classList.contains("is-success");
    wrap.classList.toggle("is-listening", active);
  });
}

function hidePairingSuccess(immediate) {
  if (pairingSuccessTimer) {
    clearTimeout(pairingSuccessTimer);
    pairingSuccessTimer = null;
  }
  ["pairingQrWrap", "connectQrWrap"].forEach((id) => {
    const wrap = el(id);
    if (!wrap) return;
    const toast = wrap.querySelector(".pairing-success-toast");
    if (toast) {
  ...
  if (!immediate && isPairingSurfaceOpen()) setPairingListening(true);
}

function showConnectedBanner(name) {
  const banner = el("pairingConnectedBanner");
  const label = name || "New device";
  if (banner) {
    banner.textContent = `${label} connected`;
    banner.classList.remove("hidden", "is-out");
    banner.classList.add("is-in");
  }
  const pill = el("deviceCountPill");
  if (pill) pill.classList.add("just-connected");
  ...
  connectedBannerTimer = setTimeout(() => {
    ...
    connectedBannerTimer = setTimeout(() => {
      ...
      if (pill) pill.classList.remove("just-connected");
    }, 350);
  }, 6000);
}

function showPairingSuccess(name) {
  const wrap = activePairingWrap();
  const label = name || "New device";
  showConnectedBanner(label);
  if (!wrap) return;
  const toast = wrap.querySelector(".pairing-success-toast");
  const nameEl = wrap.querySelector(".pairing-success-name");
  if (!toast || !nameEl) {
    if (isModalOpen("connectDeviceModal")) closeModal("connectDeviceModal");
    return;
  }
  wrap.classList.remove("is-listening");
  wrap.classList.add("is-success");
  nameEl.textContent = label;
  toast.classList.remove("hidden", "is-out");
  toast.classList.add("is-in");
  if (pairingSuccessTimer) clearTimeout(pairingSuccessTimer);
  pairingSuccessTimer = setTimeout(() => {
    toast.classList.remove("is-in");
    toast.classList.add("is-out");
    pairingSuccessTimer = setTimeout(() => {
      const closeConnect = isModalOpen("connectDeviceModal");
      hidePairingSuccess(true);
      if (closeConnect) closeModal("connectDeviceModal");
    }, 350);
  }, 1500);
}
...
el("settingsBtn").addEventListener("click", async () => {
  ...
  el("updateStatusText").textContent = "Compares GitHub Releases or your hosted JSON against this app’s baked-in version.";
...
async function runUpdateCheck({ silent = false } = {}) {
  ...
  try {
    ...
    if (!data.enabled) {
      if (!silent) el("updateStatusText").textContent = "Not configured — add a GitHub owner/repo or an update JSON URL above.";
      return;
    }
    if (!data.reachable) {
    ...
    const messages = [];
    if (data.app_update_available) {
      messages.push(data.message || `App update available: ${data.remote_app_version}`);
    }
    ...
    if (!silent) {
      const status = el("updateStatusText");
      status.replaceChildren();
      status.append(hasUpdate ? messages.join(" · ") : "You're up to date.");
      if (hasUpdate && data.notes_url) {
        status.append(" — ");
        const link = document.createElement("a");
        link.href = data.notes_url;
        link.target = "_blank";
        link.rel = "noopener noreferrer";
        link.textContent = data.notes_url;
...
async function loadPairedDevices() {
  try {
    ...
    if (container) container.innerHTML = '<p class="muted small">Could not load paired devices.</p>';
  }
}

function maybeAnnounceNewPair(devices) {
  const list = Array.isArray(devices) ? devices : [];
  const tokens = list.map((d) => d.token);
  const prev = lastPairedTokens;
  lastPairedTokens = tokens;
  if (!prev || !isPairingSurfaceOpen()) return;
  const newcomers = list.filter((d) => d.token && prev.indexOf(d.token) === -1);
  if (!newcomers.length) return;
  newcomers.sort((a, b) => (b.paired_at || 0) - (a.paired_at || 0));
  showPairingSuccess(newcomers[0].name || "New device");
}
...
el("regeneratePinBtn").addEventListener("click", async () => {
  ...
  await loadPairingInfo();
  setPairingListening(isPairingSurfaceOpen());
});
