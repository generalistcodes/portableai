...
class OllamaClient:
    ...
    def _pull_model_once(self, name: str, timeout: int | None = None) -> None:
        """Single POST /api/pull (stream=False). Raises on failure."""
        try:
            resp = requests.post(
                self._url("/api/pull"),
    ...
    def iter_pull_model(
        ...
        for attempt in range(1, PULL_MAX_ATTEMPTS + 1):
            try:
                self._pull_model_once(name, timeout=timeout)
                yield {"pulled": name}
                return
    ...
    def pull_model(
        ...
        *,
        ...
        Byte-level download progress is not reported here: Ollama's
        /api/pull supports stream=True NDJSON progress events, but this
        client uses stream=False for simplicity.
        """
        for event in self.iter_pull_model(name, timeout=timeout):
