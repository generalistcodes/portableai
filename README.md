...
From a terminal (read `install.sh` first — it is the only curl-pipe
installer; the website links here rather than hosting a copy):

```bash
curl -fsSL https://raw.githubusercontent.com/generalistcodes/portableai/HEAD/install.sh | sh
```

That downloads the Linux AppImage or macOS `.dmg` into the **current
directory** and stops. It does not launch the server.
...
instead — e.g. `update-manifest.json` — shaped like:
